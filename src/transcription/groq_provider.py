...
MAX_429_RETRIES = 3
BASE_RETRY_DELAY_SEC = 30  # Exponential backoff base: 30s, 60s, 120s
...
class GroqTranscriptionProvider:
    ...
    def __init__(self, settings=None):
        ...
        self.max_429_retries = getattr(self.settings, 'groq_max_429_retries', MAX_429_RETRIES)
    ...
    def _transcribe_standard(self, audio_path: Path, language: str) -> RawTranscript:
        """Standard transcription without chunking, with 429 retry."""
        url = GROQ_API_URL
        ...
        for attempt in range(self.max_429_retries + 1):
            ...
            if response.status_code == 429 and attempt < self.max_429_retries:
                ...
                logger.warning(
                    "Groq 429 rate limit hit, retrying in %ds (attempt %d/%d)",
                    retry_after,
                    attempt + 1,
        ...
        raise WhisperError(
            f"Groq API 429 rate limit after {self.max_429_retries} retries"
        )
